 ...
 * returns 0 on success */
int ClientSetPrivateKey(const char* privKeyName, int userEcc, void* heap, const char* tpmKeyAuth)
{
    int ret = 0;
    ...
    if (privKeyName == NULL) {
        if (userEcc) {
            ...
            ret = wolfSSH_ReadKey_buffer(hanselPrivateRsa, hanselPrivateRsaSz,
                    WOLFSSH_FORMAT_ASN1, &userPrivateKey, &userPrivateKeySz,
                    &userPrivateKeyType, &userPrivateKeyTypeSz, heap);
	        ESP_LOGE("CMN", "Error setting private key %d", ret);
        ...
        ret = wolfSSH_ReadKey_file(privKeyName,
                ...
                &isPrivate, heap);
        ESP_LOGE("CMN", "Error setting private key2 %d", ret);
    #else
